$( function() {
    //$(document).on('load','div[id^="TabContainer_"]',function(){
    ...
    // Used below line to enable the tabs functionality
    $('div[id^="TabContainer_"]').tabs();
   ...
   // Actual addTab function: adds new tab using the input from the form above
    function addTab(event,data) {
      ...
      var label = data.MPM_name,
       ...
       if($("#"+label).length==0){
            tabs.find( "ul[data-tabid='"+$(event.target).attr('data-location')+"']" ).append( li );
    ...
    function addDynamicTabByHtml(controlData){
        console.log(controlData)
        var tabs=$('#'+controlData.responseData.tablocation).tabs();
                     var label = controlData.responseData.element_name,
                    id = label+"_element",
                    li = "<li><a href='#"+id+"'>"+label+"</a> <span data-location='"+controlData.responseData.tablocation+"' class='ui-icon ui-icon-close' id= 'close-tab' role='presentation'>Remove Tab</span></li>",
                    tabContentHtml = "<div id='" + id + "'>"+controlData.html+"</div>" ;
                    //html = $.parseHTML( tabContentHtml );
                    console.log(tabs);
                   if($("#"+label).length==0){
                        console.log(tabs.find( "ul[data-tabid='"+controlData.responseData.tablocation+"']" ))
                        tabs.find( "ul[data-tabid='"+controlData.responseData.tablocation+"']" ).append( li );
                        //tabs.append( "<div id='" + id + "'></div>" );
                        tabs.append(tabContentHtml);
    ...
    function addDynamicTab(controldata){
        ...
        var tabs=$('#'+controlData.responseData.datalocation).tabs();
                    ...
                    //html = $.parseHTML( tabContentHtml );
                   if($("#"+label).length==0){
                        tabs.find( "ul[data-tabid='"+controlData.responseData.datalocation+"']" ).append( li );
                        //tabs.append( "<div id='" + id + "'></div>" );
                        tabs.append(tabContentHtml);
                        //tabs.tabs( "option", "show", { effect: "blind", duration: 800 } );
                        //$('#'+id).load(data.MPM_URL, data.MPM_stream);
...
function tabsCount(tabsId){
    var index=0;
    index = $(tabsId).find("li[aria-controls^=Tab_]").length;
    return index;
}
